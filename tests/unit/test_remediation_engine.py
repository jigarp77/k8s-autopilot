...
from autopilot.config import OperatorConfig, OperatorMode, RateLimitConfig
from autopilot.engines.approval_engine import ApprovalDecision
...
from autopilot.engines.remediation_engine import RemediationEngine, RemediationOutcome
from autopilot.remediations.registry import ActionResult
...
class TestRemediationEngine:
    ...
    async def test_auto_mode_executes_safe_action(self, deps):
        ...
        with patch("autopilot.engines.remediation_engine.registry") as mock_reg:
            ...
            result = await eng.handle(
                _make_diagnosis("restart_pod", is_safe=True),
                "default",
                "pod-x",
                "CrashLoopBackOff",
            )
