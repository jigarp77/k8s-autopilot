"""
autopilot/engines/diagnosis_engine.py
───────────────────────────────────────
Claude AI diagnosis with structured output.
"""
...
from autopilot.engines.context_collector import NodeContext, PodContext
...
class Diagnosis:
    ...
    @property
    def top_action(self) -> RemediationAction | None:
...
{
  "trigger_category": "<one of: CrashLoopBackOff|OOMKilled|ImagePullError|PendingScheduling|\
LivenessProbeFailure|NodeNotReady|NodeDiskPressure|NodeMemoryPressure|DeploymentStalled|\
ConfigurationError|DependencyFailure|Unknown>",
  ...
  "recommended_actions": [
    {
      "action": "<registry_key: restart_pod|delete_pod|scale_deployment|rollback_deployment|\
restart_deployment|cordon_node|drain_node|increase_memory_limit|no_action>",
...
Safe auto-actions (is_safe=true): restart_pod, delete_pod (only if in CrashLoop), \
scale_deployment (only to reduce replicas for OOM)
Always require approval (is_safe=false): rollback_deployment, cordon_node, drain_node, \
...
class DiagnosisEngine:
    ...
    async def diagnose_pod(self, context: PodContext, trigger: str) -> Diagnosis:
    ...
    async def diagnose_node(self, context: NodeContext, trigger: str) -> Diagnosis:
    ...
    async def _diagnose(
        ...
        if len(user_prompt) > 100_000:
            logger.warning(
                "Context for %s is %d chars — truncating to 100k",
                resource_id,
                len(user_prompt),
            )
