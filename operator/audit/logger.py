"""
operator/audit/logger.py
──────────────────────────
Persistent audit trail backed by SQLite.

Every remediation attempt is recorded with full context:
  • resource, namespace, trigger, action
  • diagnosis (JSON)
  • outcome
  • timing
  • tokens used

The audit DB can be mounted as a PVC for persistence across operator restarts.
"""
...
class AuditLogger:
            ...
            cur = self._conn.execute(
                ...
                INSERT INTO audit_log
                    (resource_id, namespace, name, trigger, action, outcome,
                     diagnosis, action_output, started_at, completed_at, decided_by, tokens_used)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    record.resource_id,
    ...
    def query(
        ...
        # crude: use LIMIT/OFFSET — fine for operator scale
        ...
        if name:      clauses.append("name = ?");      params.append(name)
        if trigger:   clauses.append("trigger = ?");   params.append(trigger)
        if outcome:   clauses.append("outcome = ?");   params.append(outcome)
    ...
    def summary(self, since_hours: int = 24) -> dict:
        ...
        return {
            "total": row[0], "executed": row[1], "failed": row[2],
            "rejected": row[3], "dry_run": row[4], "tokens_used": row[5],
