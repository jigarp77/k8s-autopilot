"""
operator/engines/context_collector.py
────────────────────────────────────────
Collects all available context about a failing resource before diagnosis:

  • Pod logs (current + previous container if restarted)
  • Pod events (Warning events from K8s event stream)
  • Pod resource requests/limits vs node allocatable
  • Node conditions and status
  • Namespace resource quotas
  • Related deployment / replicaset status
...
class PodContext:
    ...
    current_logs:      dict[str, str] = field(default_factory=dict)    # container → logs
    previous_logs:     dict[str, str] = field(default_factory=dict)    # container → prev logs
    ...
    def to_text(self) -> str:
        """Render all context as a structured text block for the AI prompt."""
        lines = [
            ...
            f"Owner       : {self.owner_kind}/{self.owner_name}" if self.owner_name else "Owner: none",
        ...
        if self.containers:
            ...
            for c in self.containers:
                lines.append(
                    f"  {c['name']}: state={c.get('state')} restarts={c.get('restarts', 0)} "
                    f"image={c.get('image', 'unknown')}"
                )
                if c.get("last_state"):
                    lines.append(f"    last_state: {c['last_state']}")
                if c.get("exit_code") is not None:
                    lines.append(f"    exit_code: {c['exit_code']} reason: {c.get('exit_reason', '')}")
            lines.append("")
        ...
        if self.namespace_quotas:
            ...
            for qname, qdata in self.namespace_quotas.items():
                lines.append(f"  {qname}: hard={qdata.get('hard',{})} used={qdata.get('used',{})}")
            lines.append("")
...
class ContextCollector:
    ...
    Called by the diagnosis engine before invoking Claude.
    """
    ...
    async def collect_pod_context(self, namespace: str, pod_name: str) -> PodContext:
        ...
        for cs in (pod.status.container_statuses or []):
            container_spec = next(
                (c for c in pod.spec.containers if c.name == cs.name), None
            )
            ...
            if container_spec and container_spec.resources:
                ctx.resource_requests = {
                    k: str(v)
                    for k, v in (container_spec.resources.requests or {}).items()
                }
                ctx.resource_limits = {
                    k: str(v)
                    for k, v in (container_spec.resources.limits or {}).items()
                }
        ...
        # Collect logs for every container
        for c in pod.spec.containers:
            ...
            if ctx.restart_count > 0:
                ctx.previous_logs[c.name] = await self._k8s.get_pod_logs(
                    pod_name, namespace, container=c.name,
        ...
        logger.debug(
            ...
            namespace, pod_name, len(ctx.containers), len(ctx.events),
        )
        return ctx
    ...
    @staticmethod
    def _state_string(state: Optional[client.V1ContainerState]) -> str:
        if not state:
            return "unknown"
        if state.running:
