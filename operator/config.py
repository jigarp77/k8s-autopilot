...
operator/config.py
──────────────────
Centralised, validated configuration for K8s Autopilot.

Sources (in priority order):
  1. Environment variables (CI/CD, K8s Secrets)
  2. ConfigMap-mounted file  (/etc/autopilot/config.yaml)
  3. Built-in defaults
"""
...
class OperatorMode(str, Enum):
    DRY_RUN = "dry-run"    # log only, no actions
    SUGGEST  = "suggest"   # notify via Slack, never auto-act
    AUTO     = "auto"      # act immediately on allowed triggers
    APPROVAL = "approval"  # always require Slack approval
...
class SlackConfig:
    ...
    webhook_url:      str  = ""
    timeout_seconds:  int  = 300   # approval window before auto-reject
    enabled:          bool = False
...
class RateLimitConfig:
    ...
    circuit_breaker_timeout:   int = 600  # seconds before half-open retry
...
class TriggerRule:
    ...
    min_restart_count:     int   = 3      # for CrashLoopBackOff: minimum restarts before acting
    cooldown_after_action: int   = 300    # seconds
...
class OperatorConfig:
    ...
    target_namespaces: list[str]     = field(default_factory=list)   # empty = all
    ignored_namespaces: list[str]    = field(default_factory=lambda: ["kube-system", "kube-public"])
    watched_labels:    dict[str,str] = field(default_factory=dict)   # empty = all
    ...
    # Rules (overridden by AutopilotPolicy CRD at runtime)
    default_rules: list[TriggerRule] = field(default_factory=list)

    # Context collection
    log_lines:          int  = 200   # lines to pull per container
    metrics_window_min: int  = 15    # minutes of metrics history
    ...
    def load(cls) -> OperatorConfig:
        ...
        logger.info(
            ...
            cfg.mode, cfg.target_namespaces or "all",
            cfg.slack.enabled, cfg.pagerduty.enabled,
        )
        return cfg
    ...
    def get_rule(self, trigger: str) -> Optional[TriggerRule]:
