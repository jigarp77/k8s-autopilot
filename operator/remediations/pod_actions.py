...
async def restart_pod(
    ...
    **kwargs,
    ...
    if not pod:
        return ActionResult(
            success=False, action="restart_pod", dry_run=dry_run,
            message=f"Pod {namespace}/{name} not found — may have already been replaced",
        )

    # Only restart if there's a controller to recreate it
    owner_refs = pod.metadata.owner_references or []
    if not owner_refs:
        return ActionResult(
            success=False, action="restart_pod", dry_run=dry_run,
            message=f"Pod {namespace}/{name} has no owner — deleting an unmanaged pod would lose it permanently",
        )

    if dry_run:
        return ActionResult(
            success=True, action="restart_pod", dry_run=True,
    ...
    # Wait briefly for a replacement to appear
...
async def delete_pod(
    ...
    **kwargs,
    ...
    if dry_run:
        return ActionResult(
            success=True, action="delete_pod", dry_run=True,
...
@registry.register(
    ...
    safe_auto=False,   # force delete can leave orphaned volumes
)
async def force_delete_pod(
    ...
    **kwargs,
    ...
    if dry_run:
        return ActionResult(
            success=True, action="force_delete_pod", dry_run=True,
            message=f"[DRY-RUN] Would force-delete pod {namespace}/{name}",
        )

    ok = await k8s.delete_pod(name, namespace, grace_period=0)
    return ActionResult(
        success=ok, action="force_delete_pod",
        message=(
            f"Force-deleted pod {namespace}/{name}"
            if ok else
            f"Failed to force-delete pod {namespace}/{name}"
        ),
    )
