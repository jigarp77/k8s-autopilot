"""
operator/utils/circuit_breaker.py
────────────────────────────────────
...
States:
  ...
  OPEN      → all remediations blocked; waits for timeout
  HALF_OPEN → one trial remediation allowed; success → CLOSED, fail → OPEN
"""
...
class CircuitBreaker:
    ...
    timeout_seconds:     int   = 600  # seconds before half-open retry
    name:                str   = "default"
    ...
    def is_open(self) -> bool:
        """Return True if remediations should be blocked."""
    ...
    def record_failure(self) -> None:
        ...
        if self._state == CBState.HALF_OPEN:
            logger.warning(
                "[CircuitBreaker:%s] Failure during HALF_OPEN → re-opening", self.name
            )
        ...
        if self._state == CBState.CLOSED and self._failure_count >= self.failure_threshold:
            logger.error(
                ...
                self.name, self._failure_count, self.timeout_seconds,
            )
            self._state        = CBState.OPEN
            self._total_opens += 1
